...
cadastrarDespesa = () => {
	...
	if(despesa.validarDados()) {
		...
		document.getElementById('modal-titulo').innerHTML = 'Erro ao registrar despesa!'
		document.getElementById('modal-titulo-div').className = 'modal-header text-danger'
		document.getElementById('modal-conteudo').innerHTML = 'Podem existir campos obrigatórios (*) que não foram preenchidos.'
		document.getElementById('modal-btn').innerHTML = 'Voltar'
...
listaDespesas = (despesas = Array(), filtro = false) => {
    ...
    despesas.forEach(function(d) {
		...
		linha.insertCell(3).innerHTML = d.valor.replace('.', ',') + ' ' + 'R$'
...
pesquisarDespesa = () => {
    ...
    if (ano == '') {
		...
		// dialog de erro
		$('#modalPesquisarDespesa').modal('show')
